          ...
          >
            ...
            {isCodeMessage && prismLoaded ? (
              ...
              <div className="md:pl-4 pl-3 pb-4 md:pb-2 pt-2 pr-3 md:pr-20 min-w-32 md:min-w-0 relative">
                {/* Fallback plain view (before Prism loads). Show copy on desktop too. */}
                {isCodeMessage && !isMobile() && (
                  <button
                    onClick={handleCopy}
                    className="absolute top-2 right-2 text-xs px-2 py-1 bg-zinc-600 hover:bg-zinc-500 text-gray-200 rounded transition-all duration-200 flex items-center gap-1"
                    title="Copy code"
                  >
                    {copied ? (
                      <>
